function [eout,thresh,gv_45,gh_135] = edge3ch(varargin)
%edge3ch Find edges in intensity image.
%   edge3ch takes an intensity or a binary image I as its input, and returns a
...
if strcmp(method,'canny')
    ...
    % Calculate gradients using a derivative of Gaussian filter
        [dx, dy] = smoothGradient(a(:,:,ch), sigma);
        ch_dx(:,:,ch) = dx;
        ch_dy(:,:,ch) = dy;
        % Calculate Magnitude of Gradient
        ch_magGrad(:,:,ch) = hypot(dx, dy);
    end
    
    [magGrad,idx] = max(ch_magGrad,[],3);
    for y=1:size(idx,1)
        for x=1:size(idx,2)
            dx(y,x) = ch_dx(y,x,idx(y,x));
            dy(y,x) = ch_dy(y,x,idx(y,x));
        end
    end
