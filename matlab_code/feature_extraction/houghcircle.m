function [y0detect,x0detect,Accumulator] = houghcircle(Imbinary,r,thresh,region,suppression_alpha)
...
% alpha = 0.5;
% d_suppress = round(alpha*r);
d_suppress = round(suppression_alpha*r);
d_suppress = d_suppress - (mod(d_suppress,2)==0);
% hpeaks = houghpeaks(Accumulator, 100, 'Threshold', thresh, 'NHoodSize', [11 11]);
hpeaks = houghpeaks(Accumulator, 100, 'Threshold', thresh, 'NHoodSize', [d_suppress d_suppress]);
% hpeaks = houghpeaks(Accumulator, 'Threshold', thresh, 'NHoodSize', [1 1]);
